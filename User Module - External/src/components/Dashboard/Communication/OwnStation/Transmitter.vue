    ...
    <div class="card">
      ...
      <form @submit.prevent="handleSubmit">
        ...
        <div class="form-row">
          ...
          <select v-model="formData.transmitter.ownership" class="form-select" required>
          ...
          <input
            v-if="formData.transmitter.ownership === 'Own by Other Party'"
            type="text"
            v-model="formData.transmitter.owner"
            class="form-control"
            :required="formData.transmitter.ownership === 'Own by Other Party'"
        ...
        <template v-if="formData.transmitter.ownership === 'Own'">
          <!-- Valuation / Status of usage -->
          <div class="form-row">
            <label>Status of Usage:</label>
            <select v-model="formData.transmitter.status" required>
          ...
          <div class="form-row">
            ...
            <select v-model="formData.transmitter.valuation" required @change="handleValuationChange">
              <option disabled value="">-- Select --</option>
              <option>Yes</option>
              <option>No</option>
            </select>
          ...
          <div class="form-row">
            ...
            <input
              type="number"
              v-model="formData.transmitter.valuationValue"
              :disabled="formData.transmitter.valuation === 'No'"
              required
              step="0.001"
              min="0"
          ...
          <div class="form-row">
            ...
            <input
              type="date"
              v-model="formData.transmitter.valuationDate"
              :disabled="formData.transmitter.valuation === 'No'"
              required
            />
          </div>
          ...
          <div class="form-row">
            ...
            <input
              type="text"
              v-model="formData.transmitter.remarks"
              maxlength="60"
            />
          </div>
        </template>
        ...
        <template v-else-if="formData.transmitter.ownership === 'Own by Other Party'">
...
export default {
  ...
  methods: {
    validateDecimal(e, decimals) {
      ...
      } else if (e.target.id in this.formData.transmitter) {
        this.formData.transmitter[e.target.id] = value;
      }
    },
    handleValuationChange() {
      if (this.formData.transmitter.valuation === "No") {
        this.formData.transmitter.valuationValue = "1";
        this.formData.transmitter.valuationDate = new Date().toISOString().split("T")[0];
      } else {
        this.formData.transmitter.valuationValue = "";
        this.formData.transmitter.valuationDate = "";
      }
    },
    handleSubmit() {
      ...
      if (!this.formData.transmitter.ownership.trim()) {
        alert("Ownership is required.");
        return;
      }
      if (
        this.formData.transmitter.ownership === "Own" &&
        !this.formData.transmitter.usage.trim()
      ) {
