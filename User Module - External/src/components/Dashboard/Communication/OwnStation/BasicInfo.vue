<template>
  <div class="container mt-3">
    <form class="card shadow-sm p-4 own-station-card" @submit.prevent="submitForm">
      ...
      <!-- Name of the Station -->
      <div class="form-row">
        <label for="name"><b>Name of the Station</b></label>
      ...
      <div class="location-section">
        ...
        <div class="inline-labels compact">
          ...
          <span>Area (km²)</span>
          <span>Ownership</span>
          <span v-if="formData.ownStation.landOwnership === 'Own by Other Party'">Land Owner</span>
        </div>
        <div class="inline-fields">
          <select v-model="formData.ownStation.unit" @change="convertToKm2" class="form-select" required>
          ...
          <input
            type="text"
            v-model="formData.ownStation.area"
            @input="onAreaInput"
            class="form-control"
            required
          />
          ...
          <input
            type="text"
            :value="formData.ownStation.areaKm"
            readonly disabled
            class="form-control"
          />

          <select v-model="formData.ownStation.landOwnership" class="form-select" required>
          ...
          <input
            v-if="formData.ownStation.landOwnership === 'Own by Other Party'"
            type="text"
            v-model="formData.ownStation.landOwner"
            class="form-control"
            :required="formData.ownStation.landOwnership === 'Own by Other Party'"
          />
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    onAreaInput(event) {
      ...
      const parts = value.split('.');
      if (parts.length > 2) value = parts[0] + '.' + parts.slice(1).join('');
      this.formData.ownStation.area = value;
      this.convertToKm2();
    },

    convertToKm2() {
      const area = parseFloat(this.formData.ownStation.area);
      if (!isNaN(area)) {
        const unit = this.formData.ownStation.unit;
        let km2 = 0;
        switch (unit) {
        ...
        this.formData.ownStation.areaKm = km2.toFixed(6);
      }
    },
    ...
    submitForm() {
      ...
      // ownStation validation
      const ownStation = this.formData.ownStation;
      if (!ownStation.unit || !ownStation.area || !ownStation.landOwnership || (ownStation.landOwnership === 'Own by Other Party' && !ownStation.landOwner)) {
        alert("Please fill all required fields in Own Station section!");
        return;
      }
