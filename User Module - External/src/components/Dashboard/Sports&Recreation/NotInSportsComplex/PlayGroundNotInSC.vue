<template>
  <div class="container mt-3">
    <form class="card shadow-sm p-4 playground-card" @submit.prevent="submitForm">
      ...
      <div class="location-section">
        ...
        <div class="inline-labels compact">
          ...
          <span>Area (km²)</span>
          <span>Ownership</span>
          <span v-if="formData.playground.landOwnership === 'Own by Other Party'">Land Owner</span>
        </div>
        <div class="inline-fields">
          <select v-model="formData.playground.unit" @change="convertToKm2" class="form-select" required>
          ...
          <input
            type="text"
            v-model="formData.playground.area"
            @input="onAreaInput"
            class="form-control"
            required
          />
          ...
          <input
            type="text"
            :value="formData.playground.areaKm"
            readonly disabled
            class="form-control"
          />

          <select v-model="formData.playground.landOwnership" class="form-select" required>
          ...
          <input
            v-if="formData.playground.landOwnership === 'Own by Other Party'"
            type="text"
            v-model="formData.playground.landOwner"
            class="form-control"
            :required="formData.playground.landOwnership === 'Own by Other Party'"
          />
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    onAreaInput(event) {
      ...
      const parts = value.split('.');
      if (parts.length > 2) value = parts[0] + '.' + parts.slice(1).join('');
      this.formData.playground.area = value;
      this.convertToKm2();
    },

    convertToKm2() {
      const area = parseFloat(this.formData.playground.area);
      if (!isNaN(area)) {
        const unit = this.formData.playground.unit;
        let km2 = 0;
        switch (unit) {
        ...
        this.formData.playground.areaKm = km2.toFixed(6);
      }
    },

    submitForm() {
      // Required base fields
      const requiredFields = ["name", "ownership", "sport", "playArea"];
      ...
      const playground = this.formData.playground;
      if (!playground.unit || !playground.area || !playground.landOwnership || 
          (playground.landOwnership === 'Own by Other Party' && !playground.landOwner)) {
        alert("Please fill all required fields in Playground section!");
        return;
      }
