<template>
  <div class="container mt-3">
    <form class="card shadow-sm p-4 jogging-path-card" @submit.prevent="submitForm">
      ...
      <div class="location-section">
        ...
        <div class="inline-labels compact">
          ...
          <span>Area (km²)</span>
          <span>Ownership</span>
          <span v-if="formData.joggingPath.landOwnership === 'Own by Other Party'">Land Owner</span>
        </div>
        <div class="inline-fields">
          <select v-model="formData.joggingPath.unit" @change="convertToKm2" class="form-select" required>
          ...
          <input
            type="text"
            v-model="formData.joggingPath.area"
            @input="onAreaInput"
            class="form-control"
            required
          />
          ...
          <input
            type="text"
            :value="formData.joggingPath.areaKm"
            readonly disabled
            class="form-control"
          />

          <select v-model="formData.joggingPath.landOwnership" class="form-select" required>
          ...
          <input
            v-if="formData.joggingPath.landOwnership === 'Own by Other Party'"
            type="text"
            v-model="formData.joggingPath.landOwner"
            class="form-control"
            :required="formData.joggingPath.landOwnership === 'Own by Other Party'"
          />
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    onAreaInput(event) {
      ...
      const parts = value.split('.');
      if (parts.length > 2) value = parts[0] + '.' + parts.slice(1).join('');
      this.formData.joggingPath.area = value;
      this.convertToKm2();
    },

    convertToKm2() {
      const area = parseFloat(this.formData.joggingPath.area);
      if (!isNaN(area)) {
        const unit = this.formData.joggingPath.unit;
        let km2 = 0;
        switch (unit) {
        ...
        this.formData.joggingPath.areaKm = km2.toFixed(6);
      }
    },
    ...
    submitForm() {
      ...
      // Jogging Path validation
      const joggingPath = this.formData.joggingPath;
      if (!joggingPath.unit || !joggingPath.area || !joggingPath.landOwnership || 
          (joggingPath.landOwnership === 'Own by Other Party' && !joggingPath.landOwner)) {
        alert("Please fill all required fields in Jogging Path section!");
        return;
      }
